...
#include "../msan-giovese/msan-giovese-afl.h"
#include <stdio.h>
...
#define AFL_QEMU_CPU_SNIPPET2         \
  do {                                \
                                      \
    if ((afl_entry_point              \
        && itb->pc == afl_entry_point)\
        || (!afl_entry_point          \
        && itb->pc == qmsan_ptr_main)){\
                                      \
      afl_setup();                    \
      afl_forkserver(cpu);            \
                                      \
...
unsigned char *msan_area_ptr = dummy;
...
static void afl_setup(void) {
  ...
  if (msan_id_str) {
    ...
    if (msan_area_ptr == (void *)-1) exit(1);

    if (inst_r) msan_area_ptr[0] = 1;
...
#if defined MSAN_GIOVESE && defined MSAN_GIOVESE_AFL
    if(afl_persistent_addr < load_addr){
        afl_persistent_addr += load_addr;
        fprintf(stderr, "persistent= 0x%lx\n", afl_persistent_addr);
    }
        fprintf(stderr, "2persistent= 0x%lx 0x%lx\n", afl_persistent_addr, load_addr);
#endif
...
void afl_persistent_loop() {
  ...
  if (persistent_first_pass) {
    ...
    if (is_persistent) {
      ...
      memset(msan_area_ptr, 0, MAP_SIZE);
      msan_area_ptr[0] = 1;
