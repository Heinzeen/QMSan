...
static void do_fp_st(DisasContext *s, int srcidx, TCGv_i64 tcg_addr, int size)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        ...
        TCGv_i32 sidx= tcg_const_i32(srcidx);
        TCGv_i32 sz= tcg_const_i32(size);
        gen_helper_qmsan_zero_N_if_taint_xmm(pc, tcg_addr, sidx, sz);
        tcg_temp_free_i32(sz);
        tcg_temp_free_i32(sidx);
        tcg_temp_free(pc);
    }
#endif
}
...
static void do_fp_ld(DisasContext *s, int destidx, TCGv_i64 tcg_addr, int size)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        TCGv pc = tcg_const_i64(qmsan_pc);
        TCGv_i32 didx= tcg_const_i32(destidx);
        switch(size){
            
            case 2:
                gen_helper_qmsan_check_taint_xmm_mem_l(pc, didx, tcg_addr);
            break;
            
            case 3:
                gen_helper_qmsan_check_taint_xmm_mem_d(pc, didx, tcg_addr);
            break;

            case 4:
                gen_helper_qmsan_check_taint_xmm_mem(pc, didx, tcg_addr);
            break;
        }
        tcg_temp_free_i32(didx);
        tcg_temp_free(pc);
    }
#endif
}
...
static void disas_uncond_b_imm(DisasContext *s, uint32_t insn)
{
    ...
    if (insn & (1U << 31)) {
        ...
        if (qasan_max_call_stack)
          gen_helper_qasan_shadow_stack_push(tcg_const_ptr(s->pc), cpu_reg_sp(s, 31));
#ifdef MSAN_GIOVESE
        gen_helper_qmsan_call(cpu_reg_sp(s, 31));
#endif
...
static void disas_comp_b_imm(DisasContext *s, uint32_t insn)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        TCGv pc = tcg_const_i64(qmsan_pc);
        TCGv_i32 reg= tcg_const_i32(rt);
        gen_helper_qmsan_check_reg(pc, reg);
...
static void disas_test_b_imm(DisasContext *s, uint32_t insn)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        TCGv pc = tcg_const_i64(qmsan_pc);
        TCGv_i32 reg= tcg_const_i32(rt);
        TCGv pos = tcg_const_i64(1ULL << bit_pos);
        gen_helper_qmsan_check_reg_pos(pc, reg, pos);
        tcg_temp_free(pos);
        tcg_temp_free_i32(reg);
        tcg_temp_free(pc);
    }
...
static void disas_exc(DisasContext *s, uint32_t insn)
{
    ...
    case 0:
        ...
        case 1:                                                     /* SVC */
#ifdef MSAN_GIOVESE
        gen_helper_qmsan_check_syscall(tcg_const_i64(qmsan_pc), cpu_reg(s, 8));
#endif
            gen_ss_advance(s);
...
static void disas_uncond_b_reg(DisasContext *s, uint32_t insn)
{
    ...
    case 2: /* RET */
        if (opc == 2 && rn == 30 && qasan_max_call_stack)
          gen_helper_qasan_shadow_stack_pop(cpu_reg(s, 30), cpu_reg_sp(s, 31));
        else if (opc == 1 && qasan_max_call_stack)
          gen_helper_qasan_shadow_stack_push(tcg_const_ptr(s->pc), cpu_reg_sp(s, 31));
#ifdef MSAN_GIOVESE
        if(opc == 2 && rn == 30)
            gen_helper_qmsan_ret(cpu_reg_sp(s, 31));
        else if(opc == 1)
            gen_helper_qmsan_call(cpu_reg_sp(s, 31));
#endif
...
static void disas_ldst_excl(DisasContext *s, uint32_t insn)
{
    ...
    case 0x5: /* LDAXR */
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            ...
            TCGv_i32 reg= tcg_const_i32(rt);
            TCGv_i32 sz= tcg_const_i32(size);
            gen_helper_qmsan_check_taint_reg_mem(pc, reg, tcg_addr, sz);
            tcg_temp_free_i32(sz);
            tcg_temp_free_i32(reg);
            tcg_temp_free(pc);
        }
    ...
    case 0x9: /* STLR */
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            ...
            TCGv_i32 reg= tcg_const_i32(rt);
            TCGv_i32 sz= tcg_const_i32(size);
            gen_helper_qmsan_zero_N_if_taint_reg(pc, tcg_addr, reg, sz);
            tcg_temp_free_i32(sz);
            tcg_temp_free_i32(reg);
            tcg_temp_free(pc);
        }
...
#ifdef MSAN_TAINT_ANALYSIS
        TCGv pc = tcg_const_i64(qmsan_pc);
        TCGv_i32 reg1= tcg_const_i32(rt);
        TCGv_i32 reg2= tcg_const_i32(rt2);
        TCGv_i32 sz= tcg_const_i32(size);
#endif
...
#ifdef MSAN_TAINT_ANALYSIS
            CHECK_BOUNDARIES_IF(s->pc){
                gen_helper_qmsan_check_taint_reg_mem(pc, reg1, tcg_addr, sz);
            }
#endif
            tcg_gen_addi_i64(tcg_addr, tcg_addr, 1 << size);
...
#ifdef MSAN_TAINT_ANALYSIS
            CHECK_BOUNDARIES_IF(s->pc){
                gen_helper_qmsan_check_taint_reg_mem(pc, reg2, tcg_addr, sz);
            }
#endif
...
#ifdef MSAN_TAINT_ANALYSIS
            CHECK_BOUNDARIES_IF(s->pc){
                gen_helper_qmsan_zero_N_if_taint_reg(pc, tcg_addr, reg1, sz);
            }
#endif
            tcg_gen_addi_i64(tcg_addr, tcg_addr, 1 << size);
...
#ifdef MSAN_TAINT_ANALYSIS
            CHECK_BOUNDARIES_IF(s->pc){
                gen_helper_qmsan_zero_N_if_taint_reg(pc, tcg_addr, reg2, sz);
            }
#endif
        }
...
#ifdef MSAN_TAINT_ANALYSIS
        tcg_temp_free_i32(sz);
        tcg_temp_free_i32(reg2);
        tcg_temp_free_i32(reg1);
        tcg_temp_free(pc);
#endif
    }
...
#ifdef MSAN_GIOVESE
    //consider the case in which sp was modified, instead of doing a lot computation
    //for each case, let's just update it once for all
    if(rn == 31)
        gen_helper_qmsan_set_sp(cpu_reg_sp(s, rn));
#endif
}
...
static void disas_pc_rel_adr(DisasContext *s, uint32_t insn)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        TCGv pc = tcg_const_i64(qmsan_pc);
        TCGv_i32 reg = tcg_const_i32(rd);
        gen_helper_qmsan_restore_reg(pc, reg);
        tcg_temp_free_i32(reg);
        tcg_temp_free(pc);
    }
#endif
...
static void disas_add_sub_imm(DisasContext *s, uint32_t insn)
{
    ...
    if (!setflags) {
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            TCGv pc = tcg_const_i64(qmsan_pc);
            TCGv_i32 reg = tcg_const_i32(rn);
            gen_helper_qmsan_check_flags_reg(pc, reg);
            tcg_temp_free_i32(reg);
            tcg_temp_free(pc);
        }
#endif
...
#ifdef MSAN_TAINT_ANALYSIS
    CHECK_BOUNDARIES_IF(s->pc){
        if(!(setflags && rd == 31)){//don't consider zf for now
            TCGv pc = tcg_const_i64(qmsan_pc);
            TCGv_i32 reg1= tcg_const_i32(rd);
            TCGv_i32 reg2= tcg_const_i32(rn);
            gen_helper_qmsan_check_taint_reg_reg(pc, reg1, reg2);
            tcg_temp_free_i32(reg2);
            tcg_temp_free_i32(reg1);
            tcg_temp_free(pc);
        }
...
#ifdef MSAN_GIOVESE
    if(rd == 31 && !setflags)
        gen_helper_qmsan_set_sp(cpu_reg_sp(s, rd));
#endif
...
#ifdef MSAN_TAINT_ANALYSIS
        CHECK_BOUNDARIES_IF(s->pc){
            //shadow propagation rule are, in theory, more complex
            //than the one implemented. Most of the times they seem to be
            //unneeded, but this one was causing false positives so we
            //are enforcing it for and instructions
            TCGv pc = tcg_const_i64(qmsan_pc);
            TCGv_i32 reg1= tcg_const_i32(rd);
            TCGv_i32 reg2= tcg_const_i32(rn);
            TCGv mask= tcg_const_i64(wmask);
            if(is_and)
                gen_helper_qmsan_check_taint_reg_reg_mask(pc,reg1, reg2, mask);
            else
                gen_helper_qmsan_check_taint_reg_reg(pc, reg1, reg2);
            tcg_temp_free(mask);
            tcg_temp_free_i32(reg2);
    ...
    if (sf != n || op21 || op0 || imm >= bitsize) {
        ...
        if (unlikely(imm == 0)) {
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                TCGv pc = tcg_const_i64(qmsan_pc);
                TCGv_i32 reg1 = tcg_const_i32(rd);
                TCGv_i32 reg2 = tcg_const_i32(rm);
                gen_helper_qmsan_check_taint_reg_reg(pc, reg1, reg2);
                tcg_temp_free_i32(reg2);
                tcg_temp_free_i32(reg1);
                tcg_temp_free(pc);
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                ...
                TCGv_i32 reg1 = tcg_const_i32(rd);
                TCGv_i32 reg2 = tcg_const_i32(rm);
                TCGv_i32 reg3 = tcg_const_i32(rn);
                gen_helper_qmsan_check_taint_reg_reg_reg(pc, reg1, reg2, reg3);
                tcg_temp_free_i32(reg3);
                tcg_temp_free_i32(reg2);
                tcg_temp_free_i32(reg1);
                tcg_temp_free(pc);
            }
...
static void disas_logic_reg(DisasContext *s, uint32_t insn)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        ...
        TCGv_i32 reg2 = tcg_const_i32(rm);
        TCGv_i32 reg3 = tcg_const_i32(rn);
        if((opc | (invert << 2)) == 0 || (opc | (invert << 2)) ==3) //AND
            gen_helper_qmsan_check_taint_and_reg_reg_reg(pc, reg1, reg2, reg3, cpu_reg(s, rm), cpu_reg(s, rn));
        else
            gen_helper_qmsan_check_taint_reg_reg_reg(pc, reg1, reg2, reg3);
        tcg_temp_free_i32(reg3);
        tcg_temp_free_i32(reg2);
...
static void disas_add_sub_ext_reg(DisasContext *s, uint32_t insn)
{
    ...
    if (!setflags) {
        ...
        if (sub_op) {
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                TCGv pc = tcg_const_i64(qmsan_pc);
                TCGv_i32 reg1 = tcg_const_i32(rm);
                TCGv_i32 reg2 = tcg_const_i32(rn);
                gen_helper_qmsan_check_flags_reg_reg(pc, reg1, reg2);
                tcg_temp_free_i32(reg2);
                tcg_temp_free_i32(reg1);
                tcg_temp_free(pc);
            }
...
static void disas_add_sub_reg(DisasContext *s, uint32_t insn)
{
    ...
    //friendly reminder: 31 is used for both sp and flag register
    if (!setflags) {
        if (sub_op) {
            tcg_gen_sub_i64(tcg_result, tcg_rn, tcg_rm);
        } else {
...
static void disas_data_proc_3src(DisasContext *s, uint32_t insn)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        ...
        TCGv_i32 reg2 = tcg_const_i32(rm);
        TCGv_i32 reg3 = tcg_const_i32(rn);
        TCGv_i32 reg4 = tcg_const_i32(ra);
        gen_helper_qmsan_check_taint_reg_reg_reg_reg(pc, reg1, reg2, reg3, reg4);
        tcg_temp_free_i32(reg4);
        tcg_temp_free_i32(reg3);
        tcg_temp_free_i32(reg2);
        tcg_temp_free_i32(reg1);
        tcg_temp_free(pc);
...
static void disas_adc_sbc(DisasContext *s, uint32_t insn)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        ...
        TCGv_i32 reg3 = tcg_const_i32(rn);
        gen_helper_qmsan_check_taint_reg_reg_reg(pc, reg1, reg2, reg3);
        gen_helper_qmsan_check_flags_reg_reg(pc, reg2, reg3);
        tcg_temp_free_i32(reg3);
        tcg_temp_free_i32(reg2);
        tcg_temp_free_i32(reg1);
        tcg_temp_free(pc);
...
static void disas_cc(DisasContext *s, uint32_t insn)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        TCGv pc = tcg_const_i64(qmsan_pc);
        TCGv_i32 reg = tcg_const_i32(rn);
        TCGv_i32 aux = tcg_const_i32(y);
        if(!is_imm)
            gen_helper_qmsan_check_flags_reg_reg(pc, reg, aux);
        else
            gen_helper_qmsan_check_flags_reg(pc, reg);
        tcg_temp_free_i32(aux);
        tcg_temp_free_i32(reg);
        tcg_temp_free(pc);
    }
#endif
...
static void disas_fp_compare(DisasContext *s, uint32_t insn)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        TCGv pc = tcg_const_i64(qmsan_pc);
        TCGv_i32 reg1 = tcg_const_i32(rn);
        TCGv_i32 reg2 = tcg_const_i32(rm);
        TCGv_i32 sz = tcg_const_i32((uint32_t) size + 3);
        gen_helper_qmsan_check_math_xmm_xmm_size(pc, reg1, reg2, sz);
        tcg_temp_free_i32(sz);
        tcg_temp_free_i32(reg2);
        tcg_temp_free_i32(reg1);
        tcg_temp_free(pc);
...
static void handle_fp_1src_double(DisasContext *s, int opcode, int rd, int rn)
{
    ...
    case 0x0: /* FMOV */
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            ...
            TCGv_i32 reg1 = tcg_const_i32(rd);
            TCGv_i32 reg2 = tcg_const_i32(rn);
            gen_helper_qmsan_check_taint_xmm_xmm(pc, reg1, reg2);
            tcg_temp_free_i32(reg2);
            tcg_temp_free_i32(reg1);
            tcg_temp_free(pc);
        }
                                  ...
                                  int rd, int rn, int rm)
{
    ...
    case 0x0: /* FMUL */
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            ...
            TCGv_i32 reg1 = tcg_const_i32(rd);
            TCGv_i32 reg2 = tcg_const_i32(rn);
            TCGv_i32 reg3 = tcg_const_i32(rm);
            TCGv_i32 sz = tcg_const_i32(5);
            gen_helper_qmsan_check_xmm_xmm_xmm_size(pc, reg1, reg2, reg3, sz);
            tcg_temp_free_i32(sz);
            tcg_temp_free_i32(reg3);
            tcg_temp_free_i32(reg2);
            tcg_temp_free_i32(reg1);
...
static void disas_fp_imm(DisasContext *s, uint32_t insn)
{
    ...
    imm = vfp_expand_imm(sz, imm8);
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            TCGv pc = tcg_const_i64(qmsan_pc);
            TCGv_i32 reg = tcg_const_i32(rd);
            TCGv_i32 size = tcg_const_i32(sz + 3);
            gen_helper_qmsan_restore_xmm_partial(pc, reg, size);
    ...
    if (itof) {
        ...
        case 1: /* float64 */
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                ...
                TCGv_i32 reg2 = tcg_const_i32(rn);
                TCGv_i32 sz = tcg_const_i32(6);
                gen_helper_qmsan_check_xmm_reg_size(pc, reg1, reg2, sz);
                tcg_temp_free_i32(sz);
                tcg_temp_free_i32(reg2);
                tcg_temp_free_i32(reg1);
                tcg_temp_free(pc);
                             ...
                             int imm5)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        ...
        gen_helper_qmsan_restore_xmm_partial(pc, reg, sz);
        tcg_temp_free_i32(sz);
        tcg_temp_free_i32(reg);
        tcg_temp_free(pc);
    }
...
static void disas_a64_insn(CPUARMState *env, DisasContext *s)
{
    ...
    gen_helper_qmsan_debug_print_instr(tcg_const_i64(s->pc - 4));
    //gen_helper_qmsan_debug_print_instr(cpu_reg_sp(s, 31));
#endif
// #ifdef MSAN_GIOVESE
