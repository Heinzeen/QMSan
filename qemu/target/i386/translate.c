...
{
    return CODE64(s) ? MO_64 : s->ss32 ? MO_32
                                       : MO_16;
}
...
/* do a wrapper for tcg_gen_movi_tl since we want to restore
   the taint here everytime. */

static void gen_op_tcg_gen_movi_tl(DisasContext *s, TCGv t, target_ulong val){
    
    tcg_gen_movi_tl(t, val);

#ifdef MSAN_TAINT_ANALYSIS
    CHECK_BOUNDARIES_RETURN(s->pc)
    TCGv pc = tcg_const_i64(qmsan_pc);
    TCGv_i32 tmp;
    if(t == s->T0)
        tmp = tcg_const_i32(0);
    ...
    gen_helper_qmsan_restore_temp(pc, tmp);
...
static void gen_op_mov_reg_v(DisasContext *s, TCGMemOp ot, int reg, TCGv t0)
{
...
#ifdef MSAN_TAINT_ANALYSIS
    CHECK_BOUNDARIES_RETURN(s->pc)
    // as for the other helpers, no assurance this is called with s->T0

    TCGv pc = tcg_const_i64(qmsan_pc);
    TCGv_i32 tmp;
    TCGv_i32 regnum = tcg_const_i32(reg);
    TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(ot));
    if(t0 == s->T0)
        tmp = tcg_const_i32(0);
    ...
    gen_helper_qmsan_check_taint_reg_tmp(pc, regnum, tmp, sz);
...
static inline void gen_op_mov_v_reg(DisasContext *s, TCGMemOp ot, TCGv t0, int reg)
{
    ...
    TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(ot));
    if(t0 == s->T0)
        mreg = tcg_const_i32(0);

    else if(t0 == s->T1)
        mreg = tcg_const_i32(1);

    else if(t0 == s->tmp0)
        mreg = tcg_const_i32(2);

    else    //default
        mreg = tcg_const_i32(3);

    gen_helper_qmsan_check_taint_tmp_reg(pc, mreg, regnum, sz);
    
    tcg_temp_free_i32(sz);
    tcg_temp_free(pc);
    tcg_temp_free_i32(mreg);
    tcg_temp_free_i32(regnum);
#endif
}
...
static inline void gen_op_ld_v(DisasContext *s, int idx, TCGv t0, TCGv a0)
{
#ifdef MSAN_TAINT_ANALYSIS
    TCGv pc = tcg_const_i64(qmsan_pc);
    CHECK_BOUNDARIES_IF(s->pc){
        //check if we are accessing some meaningfull region (i.e. A0 not tainted)
        TCGv_i32 reg = tcg_const_i32(3);
        gen_helper_qmsan_check_temp(pc, reg);
        tcg_temp_free_i32(reg);
    }
#endif
#ifdef MSAN_GIOVESE
    //whitelist accesses to FS, this one is needed for L/S mode
    if(s->override == R_FS)
        idx |= MO_QMSAN_IGNORE;
#endif
    ...
    //check if the value we just read was initialized (i.e. memory region previously written)
    if(t0 == s->T0)
        reg = tcg_const_i32(0);
    ...
    //3 reserved for A0 (used in load/store)
    else    //default
        reg = tcg_const_i32(4);

    TCGv_i32 dim = tcg_const_i32(idx & MO_SIZE);
    //check if we are accessing something pointed by FS (local thread stuff)
    if(s->override == R_FS)
        gen_helper_qmsan_restore_temp(pc, reg);
    else
        gen_helper_qmsan_check_taint_tmp_mem(pc, reg, a0,  dim);

    tcg_temp_free_i32(dim);
    tcg_temp_free(pc);
    tcg_temp_free_i32(reg);
#endif
}
...
#ifdef MSAN_TAINT_ANALYSIS
        TCGv pc  = tcg_const_i64(qmsan_pc);
        TCGv_i32 tmp0 = tcg_const_i32(0);
        TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(MO_64));

        gen_helper_qmsan_zero_N_if_taint_tmp(pc, s->A0, tmp0, sz);
...
{
    gen_op_tcg_gen_movi_tl(s, s->tmp0, pc);
    gen_op_jmp_v(s->tmp0);

#ifdef MSAN_TAINT_ANALYSIS
    CHECK_BOUNDARIES_RETURN(s->pc)
    TCGv mpc = tcg_const_i64(qmsan_pc);
    TCGv_i32 reg = tcg_const_i32(2);

    gen_helper_qmsan_check_temp(mpc, reg);
...
static inline void gen_string_movl_A0_ESI(DisasContext *s)
{
    ...
    CHECK_BOUNDARIES_RETURN(s->pc)
    TCGv pc  = tcg_const_i64(qmsan_pc);
    TCGv_i32 tmpA0 = tcg_const_i32(3);
    TCGv_i32 rsi = tcg_const_i32(R_ESI);
    TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(MO_64));

    gen_helper_qmsan_check_taint_tmp_reg(pc, tmpA0, rsi, sz);

    tcg_temp_free_i32(sz);
    tcg_temp_free_i32(rsi);
    tcg_temp_free_i32(tmpA0);
    tcg_temp_free(pc);
...
static inline void gen_string_movl_A0_EDI(DisasContext *s)
{
    ...
    TCGv pc  = tcg_const_i64(qmsan_pc);
    TCGv_i32 tmpA0 = tcg_const_i32(3);
    TCGv_i32 rdi = tcg_const_i32(R_EDI);
    TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(MO_64));

    gen_helper_qmsan_check_taint_tmp_reg(pc, tmpA0, rdi, sz);

    tcg_temp_free_i32(sz);
    tcg_temp_free_i32(rdi);
    tcg_temp_free_i32(tmpA0);
    tcg_temp_free(pc);
#endif
}
...
static inline void gen_op_testl_T0_T1_cc(DisasContext *s)
{
    ...
    TCGv pc  = tcg_const_i64(qmsan_pc);
    TCGv_i32 tmp0 = tcg_const_i32(0);
    TCGv_i32 tmp1 = tcg_const_i32(1);

    gen_helper_qmsan_check_test(pc, tmp0, tmp1);
...
static void gen_op_update_neg_cc(DisasContext *s)
{
    ...
    gen_op_tcg_gen_movi_tl(s, s->cc_srcT, 0);
}
...
static void gen_compute_eflags(DisasContext *s)
{
    ...
    {
        gen_op_tcg_gen_movi_tl(s, cpu_cc_src, CC_Z | CC_P);
...
static CCPrepare gen_prepare_eflags_c(DisasContext *s, TCGv reg)
{
    ...
    case CC_OP_ADCX:
    case CC_OP_ADCOX:
        return (CCPrepare){.cond = TCG_COND_NE, .reg = cpu_cc_dst, .mask = -1, .no_setcond = true};
    ...
    default:
        ...
        return (CCPrepare){.cond = TCG_COND_NE, .reg = reg, .mask = -1, .no_setcond = true};
    }
}
...
static CCPrepare gen_prepare_eflags_o(DisasContext *s, TCGv reg)
{
    ...
    case CC_OP_ADOX:
    case CC_OP_ADCOX:
        return (CCPrepare){.cond = TCG_COND_NE, .reg = cpu_cc_src2, .mask = -1, .no_setcond = true};
    case CC_OP_CLR:
    case CC_OP_POPCNT:
        return (CCPrepare){.cond = TCG_COND_NEVER, .mask = -1};
    default:
...
static CCPrepare gen_prepare_cc(DisasContext *s, int b, TCGv reg)
{
    ...
    case CC_OP_SUBB ... CC_OP_SUBQ:
        ...
        case JCC_BE:
            ...
            cc = (CCPrepare){.cond = TCG_COND_LEU, .reg = s->tmp4, .reg2 = t0, .mask = -1, .use_reg2 = true};
            break;
        ...
        fast_jcc_l:
            ...
            cc = (CCPrepare){.cond = cond, .reg = s->tmp4, .reg2 = t0, .mask = -1, .use_reg2 = true};
            break;
    ...
    slow_jcc:
        ...
        case JCC_BE:
            gen_compute_eflags(s);
            cc = (CCPrepare){.cond = TCG_COND_NE, .reg = cpu_cc_src, .mask = CC_Z | CC_C};
            break;
        case JCC_S:
        ...
        case JCC_LE:
            ...
            tcg_gen_shri_tl(reg, cpu_cc_src, 4); /* CC_O -> CC_S */
            tcg_gen_xor_tl(reg, reg, cpu_cc_src);
            cc = (CCPrepare){.cond = TCG_COND_NE, .reg = reg, .mask = CC_S | CC_Z};
            break;
        }
        break;
    }
...
static inline void gen_ins(DisasContext *s, TCGMemOp ot)
{
    ...
    /* Note: we must do this dummy write first to be restartable in
       case of page fault. */
    gen_op_tcg_gen_movi_tl(s, s->T0, 0);
    gen_op_st_v(s, ot, s->T0, s->A0);
...
static void gen_op(DisasContext *s1, int op, TCGMemOp ot, int d)
{
...
#if defined MSAN_GIOVESE && !defined MSAN_TAINT_ANALYSIS
    ot &= ~MO_QMSAN_IGNORE;
#endif

#ifdef MSAN_TAINT_ANALYSIS
    TCGv qmsan_t0, mpc;
    TCGv_i32 tmp0, tmp1, sz;
    CHECK_BOUNDARIES_IF(s1->pc){
        mpc = tcg_const_i64(qmsan_pc);
        tmp0 = tcg_const_i32(0);
        tmp1 = tcg_const_i32(1);
        sz = tcg_const_i32(MSAN_SIZE(ot));
        if (s1->prefix & PREFIX_LOCK){
            qmsan_t0 = tcg_temp_new_i64();
            //load value to a new register
            gen_op_ld_v(s1, ot, qmsan_t0, s1->A0);

            //read the shadow memory
            gen_helper_qmsan_check_taint_tmp_mem(mpc, tmp0, s1->A0, sz);
        }
    }
    ...
    switch (op)
    {
    //[qmsan] T0 and T1 ready
    case OP_ADCL:
        gen_compute_eflags_c(s1, s1->tmp4);
#ifdef MSAN_TAINT_ANALYSIS
        CHECK_BOUNDARIES_IF(s1->pc){
            gen_helper_qmsan_check_math_tmp_tmp(mpc, tmp0, tmp1);
        }
#endif
        if (s1->prefix & PREFIX_LOCK)
        {
    ...
    case OP_ANDL:
        if (s1->prefix & PREFIX_LOCK)
        {
            ...
            CHECK_BOUNDARIES_IF(s1->pc){
                ...
                gen_helper_qmsan_check_taint_and_tmp_tmp(mpc, tmp0,
                                        tmp1, qmsan_t0, s1->T1, sz);
            }
#endif
    ...
    case OP_ORL:
        if (s1->prefix & PREFIX_LOCK)
        {
            ...
            CHECK_BOUNDARIES_IF(s1->pc){
                //perform the OR operation
                gen_helper_qmsan_check_taint_or_tmp_tmp(mpc, tmp0,
                                        tmp1, qmsan_t0, s1->T1, sz);
            }
#endif
    ...
    case OP_XORL:
        if (s1->prefix & PREFIX_LOCK)
        {
            ...
            CHECK_BOUNDARIES_IF(s1->pc){
                //perform the OR operation
                gen_helper_qmsan_check_taint_xor_tmp_tmp(mpc, tmp0,
                                        tmp1, qmsan_t0, s1->T1, sz);
            }
#endif
    ...
    case OP_CMPL:
        ...
        set_cc_op(s1, CC_OP_SUBB + ot);
#ifdef MSAN_TAINT_ANALYSIS
        CHECK_BOUNDARIES_BREAK(s1->pc)
            gen_helper_qmsan_check_taint_cmp_tmp_tmp(mpc, tmp0,
                                    tmp1, s1->T0, s1->T1, sz);
#endif
        break;
    }
    ...
    CHECK_BOUNDARIES_IF(s1->pc){
        if (s1->prefix & PREFIX_LOCK){
            //write shadow back to memory
            gen_helper_qmsan_zero_N_if_taint_tmp(mpc, s1->A0, tmp0, sz);
            tcg_temp_free(qmsan_t0);
        }
        tcg_temp_free_i32(sz);
        tcg_temp_free_i32(tmp1);
        tcg_temp_free_i32(tmp0);
...
static void gen_inc(DisasContext *s1, TCGMemOp ot, int d, int c)
{
    if (s1->prefix & PREFIX_LOCK)
    {
        gen_op_tcg_gen_movi_tl(s1, s1->T0, c > 0 ? 1 : -1);
        tcg_gen_atomic_add_fetch_tl(s1->T0, s1->A0, s1->T0,
                                    s1->mem_index, ot | MO_LE);
    }
    else
    {
        if (d != OR_TMP0)
        {
            ...
            CHECK_BOUNDARIES_IF(s1->pc){
                ...
                TCGv_i32 regd = tcg_const_i32(d);
                TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(ot));
                

                gen_helper_qmsan_check_math_reg_tmp_tmp(pc, regd, tmp1, tmp1, sz);

                tcg_temp_free_i32(sz);
                tcg_temp_free_i32(regd);
                tcg_temp_free_i32(tmp1);
                tcg_temp_free(pc);
            }
#endif
        ...
        else
        {
            ...
            CHECK_BOUNDARIES_IF(s1->pc){
                ...
                gen_helper_qmsan_check_math_tmp_tmp(pc, tmp1, tmp1);
...
#ifdef MSAN_TAINT_ANALYSIS
    CHECK_BOUNDARIES_IF(s->pc){
        TCGv pc = tcg_const_i64(s->pc);
        TCGv_i32 val = tcg_const_i32(op2);
        TCGv_i32 isright = tcg_const_i32(is_right);

        gen_helper_qmsan_check_shift_tmp_imm(pc, val, isright);

        tcg_temp_free_i32(isright);
                             ...
                             bool is_right, TCGv count_in)
{
    ...
    default:
        ...
        gen_op_tcg_gen_movi_tl(s, s->tmp4, 0);
...
static void gen_shift(DisasContext *s1, int op, TCGMemOp ot, int d, int s)
{
    ...
    case OP_ROL:
            ...
            CHECK_BOUNDARIES_IF(s1->pc){
                TCGv mpc = tcg_const_i64(qmsan_pc);
                TCGv_i32 myreg = tcg_const_i32(d);
                TCGv_i32 src = tcg_const_i32(s == OR_ECX ? R_ECX: 0);
                TCGv_i32 isright = tcg_const_i32(0);
                gen_helper_qmsan_check_rot(mpc, myreg, src, s1->T1, isright);
                tcg_temp_free_i32(isright);
                tcg_temp_free_i32(src);
                tcg_temp_free_i32(myreg);
                tcg_temp_free(mpc);
            }
#endif
    ...
    case OP_ROR:
            ...
            CHECK_BOUNDARIES_IF(s1->pc){
                ...
                TCGv_i32 myreg = tcg_const_i32(d);
                TCGv_i32 src = tcg_const_i32(s == OR_ECX ? R_ECX: 0);
                TCGv_i32 isright = tcg_const_i32(1);
                gen_helper_qmsan_check_rot(mpc, myreg, src, s1->T1, isright);
                tcg_temp_free_i32(isright);
                tcg_temp_free_i32(src);
                tcg_temp_free_i32(myreg);
    ...
    case OP_SHL1:
        ...
        CHECK_BOUNDARIES_IF(s1->pc){
            ...
            TCGv_i32 src = tcg_const_i32(s == OR_ECX ? R_ECX: 0);
            TCGv_i32 isright = tcg_const_i32(0);
            gen_helper_qmsan_check_shift(mpc, myreg, src, s1->T1, isright);
            tcg_temp_free_i32(isright);
            tcg_temp_free_i32(src);
            tcg_temp_free_i32(myreg);
            tcg_temp_free(mpc);
...
static void gen_shifti(DisasContext *s1, int op, TCGMemOp ot, int d, int c)
{
    ...
    default:
        ...
        gen_op_tcg_gen_movi_tl(s1, s1->T1, c);
...
static TCGv gen_lea_modrm_1(DisasContext *s, AddressParts a)
{
    ...
    //we are using this, but let's restore it in case it remained
    //tainted from the last instruction
    gen_helper_qmsan_restore_temp(pc, tmpA0);
#endif
    TCGv ea = NULL;
    ...
    if (a.index >= 0)
    {
        ...
        {
#ifdef MSAN_TAINT_ANALYSIS
            TCGv_i32 idx = tcg_const_i32(a.index);
            gen_helper_qmsan_check_taint_tmp_tmp_reg(pc, tmpA0, tmpA0, idx, sz);
        ...
        {
#ifdef MSAN_TAINT_ANALYSIS
            TCGv_i32 bs = tcg_const_i32(a.base);
            gen_helper_qmsan_check_taint_tmp_tmp_reg(pc, tmpA0, tmpA0, bs, sz);
            tcg_temp_free_i32(bs);
#endif
    ...
    if (!ea)
    {
        gen_op_tcg_gen_movi_tl(s, s->A0, a.disp);
        ea = s->A0;
    }
    else if (a.disp != 0)
    {
    ...
    else
    {
        ...
        if (is_store)
        {
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                ...
                TCGv_i32 opt = tcg_const_i32(ot | MO_LE);

                gen_helper_qmsan_zero_N_if_taint_tmp(pc, s->A0, tmp, opt);

                tcg_temp_free_i32(opt);
                tcg_temp_free_i32(tmp);
                tcg_temp_free(pc);
            }
#endif
                           ...
                           target_ulong val, target_ulong next_eip)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        ...
        gen_helper_qmsan_check_taint_flags(pc);
                        ...
                        int modrm, int reg)
{
...
#ifdef MSAN_TAINT_ANALYSIS
    TCGv pc  = tcg_const_i64(qmsan_pc);
    TCGv_i32 regd = tcg_const_i32(reg);
    gen_helper_qmsan_save_t0(pc, s->T0, regd);
    tcg_temp_free_i32(regd);
#endif
    ...
    gen_op_mov_reg_v(s, ot, reg, s->T0);
#ifdef MSAN_TAINT_ANALYSIS
    //check t0, is it equal to before?
    //yes -> continue taint analysis (i.e. we read from memory)
    //no -> we did not change the value of the register
    gen_helper_qmsan_check_t0(pc, s->T0);
    tcg_temp_free(pc);
#endif
...
#ifdef MSAN_GIOVESE
    //CHECK_BOUNDARIES_RETURN(s->pc)
    gen_helper_qmsan_set_sp(cpu_regs[R_ESP]);
#endif
}
...
static void gen_push_v(DisasContext *s, TCGv val)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        ...
        TCGv pc  = tcg_const_i64(qmsan_pc);
        TCGv_i32 tmpA0 = tcg_const_i32(3);
        TCGv_i32 regd = tcg_const_i32(R_ESP);
        TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(MO_64));

        gen_helper_qmsan_check_taint_tmp_reg(pc, tmpA0, regd, sz);
...
#if defined MSAN_GIOVESE && !defined MSAN_TAINT_ANALYSIS
    //MO_QMSAN_IGNORE
    //TODO: consider doing this only on callee-saved registers
    gen_op_ld_v(s, d_ot | MO_QMSAN_IGNORE, s->T0, s->A0);
...
static inline void gen_stack_A0(DisasContext *s)
{
    ...
    TCGv_i32 tmpA0 = tcg_const_i32(3);
    TCGv_i32 regd = tcg_const_i32(R_ESP);
    TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(s->ss32 ? MO_32 : MO_16));
...
static void gen_pusha(DisasContext *s)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            ...
            pc = tcg_const_i64(qmsan_pc);
            tmpA0 = tcg_const_i32(3);
            TCGv_i32 regd = tcg_const_i32(R_ESP);
            TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(MO_64));
        ...
        pc  = tcg_const_i64(qmsan_pc);
        tmpA0 = tcg_const_i32(7 - i);
        TCGv_i32 opt = tcg_const_i32(d_ot | MO_LE);

        gen_helper_qmsan_zero_N_if_taint_reg(pc, s->A0, tmpA0, opt);
...
{
    TCGMemOp d_ot = mo_pushpop(s, s->dflag);
    TCGMemOp a_ot = CODE64(s) ? MO_64 : s->ss32 ? MO_32
                                                : MO_16;
    int size = 1 << d_ot;
    ...
    TCGv_i32 regd;
    CHECK_BOUNDARIES_IF(s->pc){
        //qw should check T1 too here
        TCGv_i32 tmp1 = tcg_const_i32(1);
        TCGv_i32 tmp3 = tcg_const_i32(3);
        regd = tcg_const_i32(R_ESP);
        TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(MO_64));

        gen_helper_qmsan_check_taint_tmp_reg(pc, tmp1, regd, sz);
        gen_helper_qmsan_check_taint_tmp_reg(pc, tmp3, regd, sz);
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        TCGv_i32 opt = tcg_const_i32(d_ot | MO_LE);
        regd = tcg_const_i32(R_EBP);

        gen_helper_qmsan_zero_N_if_taint_reg(pc, s->A0, regd, opt);
    ...
    if (level != 0)
    {
        ...
        for (i = 1; i < level; ++i)
        {
            ...
            TCGv_i32 tmp3;
            CHECK_BOUNDARIES_IF(s->pc){
                tmp3 = tcg_const_i32(3);
                TCGv_i32 regd = tcg_const_i32(R_EBP);
                TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(MO_64));
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                ...
                gen_helper_qmsan_check_taint_tmp_tmp(pc, tmp3, tmp1);
...
#ifdef MSAN_TAINT_ANALYSIS
            CHECK_BOUNDARIES_IF(s->pc){
                TCGv_i32 tmp2 = tcg_const_i32(2);
                TCGv_i32 opt = tcg_const_i32(d_ot | MO_LE);

                gen_helper_qmsan_zero_N_if_taint_tmp(pc, s->A0, tmp2, opt);
...
#ifdef MSAN_TAINT_ANALYSIS
        TCGv_i32 tmp3, tmp1, opt;
        CHECK_BOUNDARIES_IF(s->pc){
            tmp3 = tcg_const_i32(3);
            tmp1 = tcg_const_i32(1);
            opt = tcg_const_i32(d_ot | MO_LE);
...
#ifdef MSAN_TAINT_ANALYSIS
        CHECK_BOUNDARIES_IF(s->pc){
            gen_helper_qmsan_zero_N_if_taint_tmp(pc, s->A0, tmp1, opt);
...
static void gen_leave(DisasContext *s)
{
    ...
    CHECK_BOUNDARIES_IF(s->pc){
        ...
        gen_helper_qmsan_restore_temp(pc, tmp1);
...
static void gen_bnd_jmp(DisasContext *s)
{
    ...
    if ((s->prefix & PREFIX_REPNZ) == 0 && (s->flags & HF_MPX_EN_MASK) != 0 && (s->flags & HF_MPX_IU_MASK) != 0)
    {
...
#define SSE_FOP(x)                                  \
    {                                               \
        gen_helper_##x##ps, gen_helper_##x##pd,     \
...
#define AESNI_OP(x)                                 \
    {                                               \
        {NULL, gen_helper_##x##_xmm}, CPUID_EXT_AES \
    }
                    ...
                    target_ulong pc_start, int rex_r)
{
    ...
    if (is_xmm && !(s->flags & HF_OSFXSR_MASK) && ((b != 0x38 && b != 0x3a) || (s->prefix & PREFIX_DATA)))
    {
        goto unknown_op;
    }
    if (b == 0x0e)
    ...
    if (sse_fn_epp == SSE_SPECIAL)
    {
        ...
        case 0x32b: /* movntsd */
            ...
            {
                tcg_gen_ld32u_tl(s->T0, cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_L(0)));
...
#ifdef MSAN_TAINT_ANALYSIS
                CHECK_BOUNDARIES_BREAK(s->pc)

                TCGv pc = tcg_const_i64(qmsan_pc);
                TCGv_i32 xreg = tcg_const_i32(reg);
                TCGv_i32 tmp0 = tcg_const_i32(0);

                gen_helper_qmsan_check_taint_xmm_tmp(pc, xreg, tmp0);     

                tcg_temp_free_i32(tmp0);
                tcg_temp_free_i32(xreg);
                tcg_temp_free(pc);
        ...
        case 0x26f: /* movdqu xmm, ea */
            if (mod != 3)
            {
                ...
                //check if we are accessing some meaningfull region (i.e. A0 not tainted)
                TCGv pc = tcg_const_i64(qmsan_pc);
                TCGv_i32 ta0 = tcg_const_i32(3);
                TCGv_i32 xmm = tcg_const_i32(reg);

                gen_helper_qmsan_check_temp(pc, ta0);
                gen_helper_qmsan_check_taint_xmm_mem(pc, xmm, s->A0);

                tcg_temp_free_i32(ta0);
                tcg_temp_free_i32(xmm);
                tcg_temp_free(pc);
#endif
            }
            else
            {
                ...
                TCGv pc = tcg_const_i64(qmsan_pc);
                TCGv_i32 xmm = tcg_const_i32(reg);
                TCGv_i32 rxmm = tcg_const_i32(rm);
                gen_helper_qmsan_check_taint_xmm_xmm(pc, xmm, rxmm);
                tcg_temp_free_i32(rxmm);
                tcg_temp_free_i32(xmm);
                tcg_temp_free(pc);
#endif
            }
        ...
        case 0x310: /* movsd xmm, ea */
            if (mod != 3)
            {
                ...
                TCGv_i32 xreg = tcg_const_i32(reg);
                TCGv_i32 tmp = tcg_const_i32(0);
                //loading a quadword (2 << 3 bytes)
                TCGv_i32 dim = tcg_const_i32(3);

                gen_helper_qmsan_check_taint_tmp_mem(pc, tmp, s->A0, dim);
                gen_helper_qmsan_check_taint_xmm_tmp(pc, xreg, tmp);     
        ...
        case 0x27f: /* movdqu ea, xmm */
            if (mod != 3)
            {
                ...
                TCGv pc = tcg_const_i64(qmsan_pc);
                TCGv_i32 xmm = tcg_const_i32(reg);
                TCGv_i32 dim = tcg_const_i32(4);

                gen_helper_qmsan_zero_N_if_taint_xmm(pc, s->A0, xmm, dim);
            ...
            else
            {
                ...
                TCGv_i32 xmm = tcg_const_i32(reg);
                TCGv_i32 rxmm = tcg_const_i32(rm);
                gen_helper_qmsan_check_taint_xmm_xmm(pc, rxmm, xmm);
                tcg_temp_free_i32(rxmm);
                tcg_temp_free_i32(xmm);
                tcg_temp_free(pc);
#endif
        ...
        case 0x173:
            ...
            if (is_xmm)
            {
                gen_op_tcg_gen_movi_tl(s, s->T0, val);
                tcg_gen_st32_tl(s->T0, cpu_env,
        ...
        case 0x32a: /* cvtsi2sd */
            ...
            else
            {
                ...
                TCGv_i32 xmm = tcg_const_i32(reg);
                TCGv_i32 tmp = tcg_const_i32(0);
                gen_helper_qmsan_check_taint_xmm_tmp(pc, xmm, tmp);
                tcg_temp_free_i32(tmp);
                tcg_temp_free_i32(xmm);
                tcg_temp_free(pc);
#endif
        ...
        case 0x12d: /* cvtpd2pi */
            ...
            if (mod != 3)
            {
                ...
                CHECK_BOUNDARIES_IF(s->pc){
                    ...
                    TCGv pc = tcg_const_i64(qmsan_pc);
                    TCGv_i32 ta0 = tcg_const_i32(3);
                    //xmm_t0 != 0, we may want to change this
                    TCGv_i32 xmm = tcg_const_i32(0);
        ...
        case 0x1d7:
            ...
            tcg_gen_extu_i32_tl(cpu_regs[reg], s->tmp2_i32);
#ifdef MSAN_TAINT_ANALYSIS
            //this instruction writes the first 16 bits and zeroes the last 48.
            //since some sse instructions are not fully supported, be
            //conservative here and just restore the register
            TCGv mpc = tcg_const_i64(qmsan_pc);
            TCGv myreg = tcg_const_i64(reg);
            gen_helper_qmsan_restore_reg(mpc, myreg);
            tcg_temp_free(myreg);
            tcg_temp_free(mpc);
#endif
            break;
                ...
                else
                {
                    ...
                    case 0x34: /* pmovsxwq, pmovzxwq */
                        tcg_gen_qemu_ld_i32(s->tmp2_i32, s->A0,
                                            s->mem_index, MO_LEUL);
                        tcg_gen_st_i32(s->tmp2_i32, cpu_env, op2_offset + offsetof(ZMMReg, ZMM_L(0)));
                        break;
                    case 0x22:
                    case 0x32: /* pmovsxbq, pmovzxbq */
                        tcg_gen_qemu_ld_tl(s->tmp0, s->A0,
                                           s->mem_index, MO_LEUW);
                        tcg_gen_st16_tl(s->tmp0, cpu_env, op2_offset + offsetof(ZMMReg, ZMM_W(0)));
                        break;
                    case 0x2a: /* movntqda */
        ...
        do_0f_38_fx:
            ...
            case 0x0f2: /* andn Gy, By, Ey */
                if (!(s->cpuid_7_0_ebx_features & CPUID_7_0_EBX_BMI1) || !(s->prefix & PREFIX_VEX) || s->vex_l != 0)
                {
                    goto illegal_op;
                }
                ot = mo_64_32(s->dflag);
            ...
            case 0x0f7: /* bextr Gy, Ey, By */
                ...
                ot = mo_64_32(s->dflag);
                {
                    ...
                    tcg_gen_movcond_tl(TCG_COND_LEU, s->A0, s->A0, bound,
                                       s->A0, bound);
                    tcg_temp_free(bound);
                    gen_op_tcg_gen_movi_tl(s, s->T1, 1);
            ...
            case 0x0f5: /* bzhi Gy, Ey, By */
                if (!(s->cpuid_7_0_ebx_features & CPUID_7_0_EBX_BMI2) || !(s->prefix & PREFIX_VEX) || s->vex_l != 0)
                {
                    goto illegal_op;
                }
                ot = mo_64_32(s->dflag);
                ...
                gen_op_tcg_gen_movi_tl(s, s->A0, -1);
        ...
        case 0x13a:
            ...
            if (b == 0xf2)
            {
#if defined ASAN_GIOVESE || defined MSAN_GIOVESE
                /* QASAN backdoor */
                gen_helper_qasan_fake_instr(cpu_regs[R_EAX], cpu_env,
                                            cpu_regs[R_EAX], cpu_regs[R_EDI],
                                            cpu_regs[R_ESI], cpu_regs[R_EDX]);

                /*this handler causes QEMU to ignore the next byte of the code,
                to fix it, we can simply decrease the pc manually*/
                s->pc--;
                break;
#endif
            }
            ...
            if (sse_fn_eppi == SSE_SPECIAL)
            {
                ...
                {
                case 0x14: /* pextrb */
                    tcg_gen_ld8u_tl(s->T0, cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_B(val & 15)));
                    if (mod == 3)
                    {
                        gen_op_mov_reg_v(s, ot, rm, s->T0);
                    }
                    ...
                    break;
                case 0x15: /* pextrw */
                    tcg_gen_ld16u_tl(s->T0, cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_W(val & 7)));
                    if (mod == 3)
                    {
                        gen_op_mov_reg_v(s, ot, rm, s->T0);
                    }
                ...
                case 0x16:
                    ...
                    break;
                case 0x17: /* extractps */
                    tcg_gen_ld32u_tl(s->T0, cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_L(val & 3)));
                    if (mod == 3)
                    {
                        gen_op_mov_reg_v(s, ot, rm, s->T0);
                    }
                ...
                case 0x20: /* pinsrb */
                    ...
                    tcg_gen_st8_tl(s->T0, cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_B(val & 15)));
                    break;
                case 0x21: /* insertps */
                    if (mod == 3)
                    {
                    ...
                    if ((val >> 0) & 1)
                        tcg_gen_st_i32(tcg_const_i32(0 /*float32_zero*/),
                                       cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_L(0)));
                    if ((val >> 1) & 1)
                        tcg_gen_st_i32(tcg_const_i32(0 /*float32_zero*/),
                                       cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_L(1)));
                    if ((val >> 2) & 1)
                        tcg_gen_st_i32(tcg_const_i32(0 /*float32_zero*/),
                                       cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_L(2)));
                    if ((val >> 3) & 1)
                        tcg_gen_st_i32(tcg_const_i32(0 /*float32_zero*/),
                                       cpu_env, offsetof(CPUX86State, xmm_regs[reg].ZMM_L(3)));
                    break;
                case 0x22:
                    if (ot == MO_32)
                    { /* pinsrd */
            ...
            if (b1)
            {
                ...
                CHECK_BOUNDARIES_IF(s->pc){
                    ...
                    TCGv pc = tcg_const_i64(qmsan_pc);
                    TCGv_i32 ta0 = tcg_const_i32(3);
                    TCGv_i32 xmm = tcg_const_i32(rm | REX_B(s));
    ...
    else
    {
        ...
        if (is_xmm)
        {
            ...
            if (mod != 3)
            {
                ...
                case 2:
                    ...
                    CHECK_BOUNDARIES_BREAK(s->pc)
                    gen_helper_qmsan_check_temp(pc, ta0);
                    gen_helper_qmsan_check_taint_xmm_mem_l(pc, xmm, s->A0);
#endif
                    break;
                case 3:
                    /* 64 bit access */
                    ...
                    CHECK_BOUNDARIES_BREAK(s->pc)
                    gen_helper_qmsan_check_temp(pc, ta0);
                    gen_helper_qmsan_check_taint_xmm_mem_d(pc, xmm, s->A0);
#endif
                    break;
                default:
                    /* 128 bit access */
        ...
        if (b == 0x2e || b == 0x2f)
        {
            ...
            CHECK_BOUNDARIES_RETURN(s->pc)
            TCGv pc = tcg_const_i64(qmsan_pc);
            TCGv_i32 xrm = tcg_const_i32(rm);
            TCGv_i32 xreg = tcg_const_i32(reg);

            gen_helper_qmsan_check_math_xmm_xmm(pc, xreg, xrm);

            tcg_temp_free_i32(xreg);
            tcg_temp_free_i32(xrm);
            tcg_temp_free(pc);
...
#ifdef MSAN_TAINT_ANALYSIS
            CHECK_BOUNDARIES_IF(s->pc){
                //managing bytes that are not explicitely managed before
                switch(b){
                    case 0x57:      //xorps
                    case 0xef:      //pxor
                        //xor with same registers (i.e. init to 0)
                        if(mod == 3 && rm == reg){

                        TCGv pc = tcg_const_i64(qmsan_pc);
                        TCGv_i32 xreg = tcg_const_i32(reg);
                        TCGv_i32 sz = tcg_const_i32(7);

                        gen_helper_qmsan_restore_xmm_partial(pc, xreg, sz);
...
#ifdef MSAN_INSTR
        // for debugging purposes, slow but usefull
        TCGv mpc = tcg_const_i64(qmsan_pc);
        gen_helper_qmsan_debug_print_instr(mpc);
        gen_helper_qmsan_debug_print_sp(cpu_regs[R_ESP]);
        tcg_temp_free(mpc);
#endif

// #ifdef MSAN_GIOVESE
//     if(qmsan_pc == qmsan_ptr_main)
//         gen_helper_qmsan_main();
// #endif
...
next_byte:
    ...
    case 0xc4: /* 3-byte VEX */
        ...
        if (s->code32 && !s->vm86)
        {
            ...
            if (prefixes & (PREFIX_REPZ | PREFIX_REPNZ | PREFIX_LOCK | PREFIX_DATA))
            {
                goto illegal_op;
            }
#ifdef TARGET_X86_64
...
reswitch:
    ...
    case 0x38 ... 0x3d:
    {
        ...
        case 2: /* OP A, Iv */
            ...
            gen_op_tcg_gen_movi_tl(s, s->T1, val);
#ifdef MSAN_TAINT_ANALYSIS
            if (op == 4 && MSAN_IS_BOGUS(val)){
                TCGv mpc = tcg_const_i64(qmsan_pc);
                TCGv myreg = tcg_const_i64(R_EAX);
                gen_helper_qmsan_restore_reg(mpc, myreg);
                tcg_temp_free(myreg);
                tcg_temp_free(mpc);
            }
    ...
    case 0x83:
    {
        ...
        gen_op_tcg_gen_movi_tl(s, s->T1, val);
        //filter out OR [...], 0x0 instructions
#if defined MSAN_GIOVESE && !defined MSAN_TAINT_ANALYSIS
        if(((op == 1) && ((mod == 0x0 && val == 0) || (mod == 0x1 && val == 0))))
            ot |= MO_QMSAN_IGNORE;
        
#endif
#ifdef MSAN_TAINT_ANALYSIS
        //filter out inlined string operations
        //check if we have an AND between reg and a bogus operand
        if (b == 0x81 && op == 4 && MSAN_IS_BOGUS(val)){
            TCGv mpc = tcg_const_i64(qmsan_pc);
            TCGv myreg = tcg_const_i64(opreg);
            gen_helper_qmsan_restore_reg(mpc, myreg);
            tcg_temp_free(myreg);
            tcg_temp_free(mpc);
        }
    ...
    case 0xf7:
        ...
        TCGv pc;
#ifdef MSAN_TAINT_ANALYSIS
        TCGv_i32 tmp0, tmp1, reax, redx, opt, sz;
        CHECK_BOUNDARIES_IF(s->pc){
            pc  = tcg_const_i64(qmsan_pc);
            tmp0 = tcg_const_i32(0);
            tmp1 = tcg_const_i32(1);
            reax = tcg_const_i32(R_EAX);
            redx = tcg_const_i32(R_EDX);
            opt = tcg_const_i32(ot | MO_LE);
            sz = tcg_const_i32(MSAN_SIZE(ot));
        }
#endif
#endif
...
#ifdef MSAN_TAINT_ANALYSIS
                    CHECK_BOUNDARIES_IF(s->pc){
                        gen_helper_qmsan_zero_N_if_taint_tmp(pc, s->A0, tmp0, opt);
                    }
#endif
                }
                else
...
#ifdef MSAN_TAINT_ANALYSIS
                CHECK_BOUNDARIES_BREAK(s->pc)
                gen_helper_qmsan_check_math_reg_tmp_tmp(pc, reax, tmp0, tmp1, sz);
#endif
                break;
            case MO_16:
                ...
                CHECK_BOUNDARIES_BREAK(s->pc)
                gen_helper_qmsan_check_math_reg_tmp_tmp(pc, reax, tmp0, tmp1, sz);
                gen_helper_qmsan_check_math_reg_tmp_tmp(pc, redx, tmp0, tmp1, sz);
#endif
                break;
            default:
            case MO_32:
...
#ifdef MSAN_TAINT_ANALYSIS
                CHECK_BOUNDARIES_BREAK(s->pc)
                gen_helper_qmsan_check_math_reg_reg_tmp(pc, reax, reax, tmp0, sz);
                gen_helper_qmsan_check_math_reg_reg_tmp(pc, redx, reax, tmp0, sz);
#endif
                break;
#ifdef TARGET_X86_64
            case MO_64:
...
#ifdef MSAN_TAINT_ANALYSIS
                CHECK_BOUNDARIES_BREAK(s->pc)
                gen_helper_qmsan_check_math_reg_reg_reg_tmp(pc, reax, reax, redx, tmp0, sz);
#endif
                break;
            default:
            case MO_32:
        ...
        tcg_temp_free_i32(reax);
        tcg_temp_free_i32(redx);
        tcg_temp_free_i32(tmp1);
        tcg_temp_free_i32(tmp0);
        tcg_temp_free_i32(opt);
        tcg_temp_free_i32(sz);
    ...
    case 0xff: /* GRP5 */
        ...
        case 2: /* call Ev */
            ...
            next_eip = s->pc - s->cs_base;
#if defined QASAN || defined MSAN_GIOVESE
            if (qasan_max_call_stack)
                gen_helper_qasan_shadow_stack_push(tcg_const_ptr(s->pc), cpu_regs[R_ESP]);
#endif

#ifdef MSAN_GIOVESE
            CHECK_BOUNDARIES_IF(s->pc){
                gen_helper_qmsan_call(cpu_regs[R_ESP]);
            }
            else
                gen_helper_qmsan_check_call_from_lib(s->T0, cpu_regs[R_ESP]);
#endif

            gen_op_tcg_gen_movi_tl(s, s->T1, next_eip);
            gen_push_v(s, s->T1);
            gen_op_jmp_v(s->T0);
#ifdef MSAN_TAINT_ANALYSIS
            CHECK_BOUNDARIES_IF(s->pc){
                ...
                gen_helper_qmsan_check_temp(pc, tmp0);
        ...
        do_ljmp:
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                ...
                gen_helper_qmsan_check_temp(pc, tmp1);
        ...
        case 6: /* push Ev */
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                ...
                gen_helper_qmsan_zero_N_if_taint_tmp(pc, s->A0, tmp0, tmp3);
    ...
    case 0x6b:
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            ...
            gen_helper_qmsan_check_math_reg_tmp_tmp(pc, myreg, tmp0, tmp1, sz);
        ...
        else
        {
            ...
            else
            {
                ...
                CHECK_BOUNDARIES_IF(s->pc){
                    ...
                    //newv was loaded with "reg", so no need to check some temp register, just go with the source
                    gen_helper_qmsan_zero_N_if_taint_reg(pc, s->A0, regd, opt);
...
#ifdef MSAN_GIOVESE
        //CHECK_BOUNDARIES_BREAK(s->pc)
        if(b == 0x88) break;        //8 bit mov, not of our concern for rsp
    ...
    case 0x1bf: /* movswS Gv, Eb */
    {
        ...
        if (mod == 3)
        {
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                pc = tcg_const_i64(qmsan_pc);
                TCGv_i32 tmp = tcg_const_i32(0);
                TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(s_ot));

                gen_helper_qmsan_widening_tmp(pc, tmp, sz);
    ...
    case 0x8d: /* lea */
        ...
        reg = ((modrm >> 3) & 7) | rex_r;
        {
            ...
            CHECK_BOUNDARIES_IF(s->pc){
                ...
                TCGv_i32 idx = tcg_const_i32(a.index);
                TCGv_i32 bs = tcg_const_i32(a.base);
                TCGv_i32 sz = tcg_const_i32(MSAN_SIZE(s->aflag));

                //we need to check only base and index (the regs)
                if(a.index >= 0){
                    gen_helper_qmsan_check_taint_tmp_reg(pc, tmpA0, idx, sz);
                    if(a.base>=0)
                        gen_helper_qmsan_check_taint_tmp_tmp_reg(pc, tmpA0, tmpA0, bs, sz);
                }
                else if(a.base >= 0 && cpu_regs[a.base])
                    gen_helper_qmsan_check_taint_tmp_reg(pc, tmpA0, bs, sz);
    ...
    case 0xa3:
    {
        ...
        gen_op_tcg_gen_movi_tl(s, s->A0, offset_addr);
        gen_add_A0_ds_seg(s);
        if ((b & 2) == 0)
        {
            gen_op_ld_v(s, ot, s->T0, s->A0);
    ...
    case 0xb8 ... 0xbf: /* mov R, Iv */
        ...
        if (dflag == MO_64)
        {
            ...
            reg = (b & 7) | REX_B(s);
            gen_op_tcg_gen_movi_tl(s, s->T0, tmp);
...
#ifdef MSAN_TAINT_ANALYSIS
                CHECK_BOUNDARIES_IF(s->pc){
                    gen_helper_qmsan_store8(cpu_env, s->A0);
                    TCGv_i64 next = tcg_const_i64(8);
                    tcg_gen_add_tl(next, s->A0, next);
                    gen_helper_qmsan_store4(cpu_env, next);
                    tcg_temp_free(next);
                }
#endif
                break;
...
#if defined QASAN || defined MSAN_GIOVESE
        if (qasan_max_call_stack)
            gen_helper_qasan_shadow_stack_pop(s->T0, cpu_regs[R_ESP]);
#endif

#ifdef MSAN_GIOVESE
        CHECK_BOUNDARIES_IF(s->pc){
            gen_helper_qmsan_ret(cpu_regs[R_ESP]);
        }
        else
            gen_helper_qmsan_check_ret_from_lib(s->T0, cpu_regs[R_ESP]);
#endif
    ...
    case 0xe8: /* call im */
    {
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            ...
            TCGv qtval = tcg_const_i64(tval);
            gen_helper_qmsan_check_call_from_lib(qtval, cpu_regs[R_ESP]);
            tcg_temp_free(qtval);
        }
#endif

        gen_op_tcg_gen_movi_tl(s, s->T0, next_eip);
        gen_push_v(s, s->T0);
        gen_bnd_jmp(s);
        gen_jmp(s, tval);
    }
    ...
    case 0x9a: /* lcall im */
    {
        ...
        gen_op_tcg_gen_movi_tl(s, s->T0, selector);
        gen_op_tcg_gen_movi_tl(s, s->T1, offset);
    }
        goto do_lcall;
    case 0xe9: /* jmp im */
        if (dflag != MO_16)
    ...
    case 0x190 ... 0x19f: /* setcc Gv */
        ...
        gen_helper_qmsan_check_taint_tmp_flag(pc, tmp0);
    ...
    bt_op:
        ...
        gen_op_tcg_gen_movi_tl(s, s->tmp0, 1);
    ...
    case 0x134: /* sysenter */
        ...
        CHECK_BOUNDARIES_IF(s->pc){
            ...
            gen_helper_qmsan_check_syscall(pc, cpu_regs[R_EAX]);
    ...
    case 0x101:
        ...
        case 0xca: /* clac */
            if (!(s->cpuid_7_0_ebx_features & CPUID_7_0_EBX_SMAP) || s->cpl != 0)
            {
                goto illegal_op;
            }
        ...
        case 0xd0: /* xgetbv */
            if ((s->cpuid_ext_features & CPUID_EXT_XSAVE) == 0 || (s->prefix & (PREFIX_LOCK | PREFIX_DATA | PREFIX_REPZ | PREFIX_REPNZ)))
            {
                goto illegal_op;
            }
    ...
    case 0x63: /* arpl or movslS (x86_64) */
...
#endif
        {
            ...
            gen_op_tcg_gen_movi_tl(s, t2, 0);
            label1 = gen_new_label();
            tcg_gen_brcond_tl(TCG_COND_GE, s->tmp0, t1, label1);
            tcg_gen_andi_tl(t0, t0, ~3);
            tcg_gen_or_tl(t0, t0, t1);
            gen_op_tcg_gen_movi_tl(s, t2, CC_Z);
            gen_set_label(label1);
            if (mod != 3)
            {
    ...
    case 0x11a:
        ...
        if (s->flags & HF_MPX_EN_MASK)
        {
            ...
            else if (mod != 3)
            {
                /* bndldx */
                AddressParts a = gen_lea_modrm_0(env, s, modrm);
                if (reg >= 4 || (prefixes & PREFIX_LOCK) || s->aflag == MO_16 || a.base < -1)
                {
                    goto illegal_op;
                }
                if (a.base >= 0)
                ...
                {
                    gen_op_tcg_gen_movi_tl(s, s->A0, 0);
                }
                gen_lea_v_seg(s, s->aflag, s->A0, a.def_seg, s->override);
                if (a.index >= 0)
                {
    ...
    case 0x1ae:
        ...
        switch (modrm)
        {
            ...
            CASE_MODRM_MEM_OP(4) : /* xsave */
                                   if ((s->cpuid_ext_features & CPUID_EXT_XSAVE) == 0 || (prefixes & (PREFIX_LOCK | PREFIX_DATA | PREFIX_REPZ | PREFIX_REPNZ)))
            {
                goto illegal_op;
            }
            gen_lea_modrm(env, s, modrm);
            ...
            {
                /* xsaveopt */
                if ((s->cpuid_ext_features & CPUID_EXT_XSAVE) == 0 || (s->cpuid_xsave_features & CPUID_XSAVE_XSAVEOPT) == 0 || (prefixes & (PREFIX_REPZ | PREFIX_REPNZ)))
                {
                    goto illegal_op;
                }
                gen_lea_modrm(env, s, modrm);
            ...
            {
                /* clflush */
                if ((s->prefix & (PREFIX_REPZ | PREFIX_REPNZ)) || !(s->cpuid_features & CPUID_CLFLUSH))
                {
                    goto illegal_op;
                }
            }
        ...
        case 0xd8 ... 0xdf: /* wrgsbase (f3 0f ae /3) */
            if (CODE64(s) && (prefixes & PREFIX_REPZ) && !(prefixes & PREFIX_LOCK) && (s->cpuid_7_0_ebx_features & CPUID_7_0_EBX_FSGSBASE))
            {
        ...
        case 0xf8: /* sfence / pcommit */
            ...
            {
                /* pcommit */
                if (!(s->cpuid_7_0_ebx_features & CPUID_7_0_EBX_PCOMMIT) || (prefixes & PREFIX_LOCK))
                {
                    goto illegal_op;
                }
                break;
...
void tcg_x86_init(void)
{
    ...
    for (i = 0; i < 6; ++i)
    {
        cpu_seg_base[i] = tcg_global_mem_new(cpu_env,
    ...
    for (i = 0; i < 4; ++i)
    {
        cpu_bndl[i] = tcg_global_mem_new_i64(cpu_env,
                                             offsetof(CPUX86State, bnd_regs[i].lb),
                                             bnd_regl_names[i]);
        cpu_bndu[i] = tcg_global_mem_new_i64(cpu_env,
...
static void i386_tr_translate_insn(DisasContextBase *dcbase, CPUState *cpu)
{
    ...
    else if ((tb_cflags(dc->base.tb) & CF_USE_ICOUNT) && ((pc_next & TARGET_PAGE_MASK) != ((pc_next + TARGET_MAX_INSN_SIZE - 1) & TARGET_PAGE_MASK) || (pc_next & ~TARGET_PAGE_MASK) == 0))
    {
