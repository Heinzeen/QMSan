                                 ...
                                 struct image_info *info)
{
    ...
    //we don't want the stack to be cleaned, but we do it for the guard page
#ifdef MSAN_GIOVESE
            msan_giovese_add_mmap(error, error + guard);
#endif
    if (error == -1) {
        perror("mmap stack");
        exit(-1);
                           ...
                           char bprm_buf[BPRM_BUF_SIZE])
{
    ...
    if (ehdr->e_type == ET_DYN) {
        ...
        load_addr = target_mmap(loaddr, hiaddr - loaddr, PROT_NONE,
                                ...
                                -1, 0);
#ifdef MSAN_GIOVESE
        msan_giovese_add_mmap(load_addr, load_addr + (((hiaddr - loaddr) / 4096) + 1 ) *4096);
#endif
        if (load_addr == -1) {
            goto exit_perror;
        }
...
#ifdef MSAN_GIOVESE_AFL
        if(afl_entry_point < info->load_addr){
            afl_entry_point += info->load_addr;
            //fprintf(stderr, "entrypoint= 0x%lx\n", afl_entry_point);
        }
#endif
      } else {
#ifdef MSAN_AFL
...
#ifdef MSAN_GIOVESE
            msan_giovese_add_mmap(error, error + vaddr_len);
#endif
            if (error == -1) {
                goto exit_perror;
            }
...
#ifdef MSAN_GIOVESE
                //not really an mmap, but it will do
                msan_giovese_add_mmap(vaddr_ef, vaddr_em);
#endif
            }
...
#ifdef MSAN_GIOVESE
    msan_giovese_set_ld(info->start_code, info->end_code);
#endif
    return;
...
int load_elf_binary(struct linux_binprm *bprm, struct image_info *info)
{
    ...
    if (elf_interpreter) {
        ...
        if (strcmp(elf_interpreter, "/usr/lib/libc.so.1") == 0
            ...
            abi_long ret = target_mmap(0, qemu_host_page_size, PROT_READ | PROT_EXEC,
                        MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
#ifdef MSAN_GIOVESE
            msan_giovese_add_mmap(ret, ret + qemu_host_page_size);
#endif
        }
#ifdef TARGET_MIPS
...
#ifdef MSAN_GIOVESE
    msan_giovese_add_mmap(bprm->p, start);
#endif
    return 0;
}
