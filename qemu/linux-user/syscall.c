...
#ifdef MSAN_GIOVESE
//msan workaround:
//the first brk is caused by libc to load thread local stuff. Since the libc
//relies on the fact that these things are 0 by default, they do not overwrite
//some of them, and we later encounter errors because of this. This solution
//is not elegant, and we should modify it, but it kinda works right now...

//Heinzeen edit; do we need this? by setting this to 1 we have no FP...
int brk_called = 1;
#endif
...
abi_long do_brk(abi_ulong new_brk)
{
    ...
    if (mapped_addr == brk_page) {
        //TODO: why did we introduce this back then...?
#ifdef MSAN_GIOVESE
        if (!brk_called){
            msan_giovese_add_mmap(brk_page, brk_page + new_alloc_size);
            brk_called = 1;
        }
#endif
        /* Heap contents are initialized to zero, as for anonymous
    ...
    || defined(TARGET_S390X)
        {
            ...
            ret = get_errno(target_mmap(v1, v2, v3,
                                        ...
                                        v5, v6));
#ifdef MSAN_GIOVESE
            msan_giovese_add_mmap(ret, ret + v2);
#endif
        }
#else
        ret = get_errno(target_mmap(arg1, arg2, arg3,
                                    ...
                                    arg6));
#ifdef MSAN_GIOVESE
        //Heinzeen: known issue here... apparently when compiling with clang
        //nothing breaks; but when you are using gcc/g++ compiled binaries
        //the heap is somehow wthitelisted if you decomment the second part 
        //of this condition
        if( ((int) arg5) != -1 || arg4 & MAP_ANONYMOUS)
            msan_giovese_add_mmap(ret, ret + arg2);
#endif
#endif
        return ret;
#endif
